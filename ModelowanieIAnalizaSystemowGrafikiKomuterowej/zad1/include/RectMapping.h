...
public:
    ...
    void getTexelCoord(const Vector3Bf &localHitPoint, const d_type::Bint m_width, const d_type::Bint m_height, d_type::Bint& row, d_type::Bint& column) const
    {
        ...
        Vector3Bf uAxis=Vector3Bf(m_plane->getNormal().y,m_plane->getInPoint().z,-m_plane->getInPoint().x);
        Vector3Bf vAxis=Vector3Bf::cross(uAxis,m_plane->getNormal());

        d_type::Bfloat u=Vector3Bf::dotProduct(localHitPoint,uAxis)-floorf(Vector3Bf::dotProduct(localHitPoint,uAxis));//*m_scale.x;
        d_type::Bfloat v =Vector3Bf::dotProduct(localHitPoint,vAxis)-floorf(Vector3Bf::dotProduct(localHitPoint,vAxis));//*m_scale.y;
